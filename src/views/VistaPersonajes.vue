<template>
    <section class="h-screen bg-no-repeat bg-cover landscape:flex landscape:flex-col landscape:justify-center landscape:items-center landscape:bg-[url('/src/assets/img/fondo.jpg')] gap-[20px]">
        ...
        <div class="flex flex-row gap-[50px]">
            <select class="w-[300px] h-[75px] rounded-[7px] text-[24px] font-bold p-[10px]" v-model="p1">
            ...
            <select class="w-[300px] h-[75px] rounded-[7px] text-[24px] font-bold p-[10px]"  v-model="p2">
                <option value="">Elige un Personaje</option>
                <option v-for="(persona, index) in personas" :key="index" :value="persona.nombre">{{ persona.nombre }}</option>
            </select>
        </div>
        <router-link to="/escenarios"><button class="bg-[#3dbc1e] h-[75px] w-[250px] rounded-[7px] font-bold text-[24px] text-[white]" @click="guardarPersonajes">Continuar</button></router-link>
